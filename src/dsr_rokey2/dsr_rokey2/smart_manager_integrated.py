...
class SmartManagerNode(Node):
    def __init__(self):
        ...
        # =========================================================
        # 2. Extended Publishers (동료분 추가 기능: 좌표 중계)
        # =========================================================
        self.coord_pubs = {
        ...
        # 개별 Dispose 명령용 (좌표 토픽과 연동될 수 있음)
        self.dispose_pubs = {
        ...
        self.get_logger().info("🤖 Smart Manager Integrated Ready (Safety + Extension)")
    ...
    def yolo_callback(self, msg):
        try:
            ...
            # [NEW] 동료분 기능: 좌표 데이터 갱신 (항상 수행)
            for obj in data:
                if "bad" in obj['name']:
                    self.detected_bad_parts[obj['name']] = obj['coords']
    ...
    def speak_assembly_result(self, state):
        ...
        elif state == "non_pass": text = "아이고, 이런. 조립이 잘못된 상태입니다. 다시 조립하셔야 합니다."
    ...
    def execute_robot_action(self, intent):
        ...
        # [Extended] 일괄 확인
             ...
             # (간략화) 기존 로직대로 단일 토픽으로 처리 가능하면 유지
        ...
        # -------------------------------------------------------------
            ...
            if target != "none":
                # [통합] 기존 단일 토픽 + 동료분 개별 토픽 둘 다 지원
                msg.data = target
                self.publisher_dispose.publish(msg)
                
                # 좌표 중계 (동료분 로직 호환)
                if target in self.detected_bad_parts and target in self.coord_pubs:
                    c_msg = Float32MultiArray()
                    c_msg.data = self.detected_bad_parts[target]
                    self.coord_pubs[target].publish(c_msg)
            ...
            # 로봇이 홈에 도착하면 robot_status 토픽이 오면서 다시 판독 활성화됨
