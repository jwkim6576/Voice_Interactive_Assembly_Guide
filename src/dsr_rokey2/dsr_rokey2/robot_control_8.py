...
class RobotController(Node):
    def __init__(self):
        ...
        self.get_logger().info("🤖 Robot Control v12 Ready (Part 3 Sorting Added)")
        self.init_thread()
    ...
    def execute_move(self, msg, action="show"):
        self.is_paused = False 
        target_name = msg.data # "part_3_bad"
        ...
        if action == "show":
            ...
            # [단일 깊이 적용]
            GLOBAL_GRIP_DEPTH = 40.0 
            
            pick_pos[2] -= GLOBAL_GRIP_DEPTH 
            pick_pos[5] += 90.0
            ...
            # ----------------------------------------------------------------
            # [NEW] 버리기 (Sorting Logic)
            # ----------------------------------------------------------------
            self.check_pause()
            
            # 3번 부품이면 전용 위치로, 아니면 기본 위치로
            if "part_3_bad" in target_name:
