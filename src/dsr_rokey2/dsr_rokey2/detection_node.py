...
from dsr_rokey2.realsense import ImgNode
from dsr_rokey2.yolo import YoloModel
...
class ObjectDetectionNode(Node):
    ...
    def _get_depth(self, x, y):
        """픽셀 좌표의 depth 값을 안전하게 읽어옵니다 (주변부 3x3 평균 사용)."""
        frame = self._wait_for_valid_data(self.img_node.get_depth_frame, "depth frame")
        
        try:
            # [수정] 한 점만 보지 않고, 주변 3x3 영역을 봅니다.
            depths = []
            for dx in range(-2, 3): # x-2 ~ x+2
                for dy in range(-2, 3): # y-2 ~ y+2
                    try:
                        d = frame[y+dy, x+dx]
                        # 0이 아닌 유효한 값(너무 먼 거리 제외)만 수집
                        if 0 < d < 2000: # 2미터 이내만 유효
                            depths.append(d)
                    except: pass
